...
Fuzzing is a technique introduced in 1990 and found failures in unix utilities by generating random inputs.
...
\section{Final project report tasks}

\begin{enumerate}
\item[] Random testing: Using the current tests,
we will change the input generation to be generic (any Unicode string or Integer, for example),
instead of tied to the specific input expected by khmer.
Hypothesis will work like a fuzzer,
and we can check for errors on input validation.
\item[] Unit tests adaptation: Current unit tests well be used as templates for new Hypothesis tests,
using the \emph{@example} decorator.
\item[] Method coverage: We'll continue to add more methods from the core data structures to the tests.
\end{enumerate}
